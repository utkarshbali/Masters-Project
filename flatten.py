...
def main():
    global worksheets
    fileName = 'CompareResultsMCMC200.xlsx'
    #fileName = sys.argv[1]
    ...
    for name in worksheets:
        ...
        nodes = sheet.cell(1,2)
        edges = sheet.cell(2,2)
        degree = sheet.cell(3,2)
        ccCount = sheet.cell(4,2)
        triangles = sheet.cell(5,2)
        connComp = sheet.cell(6,2)
        overlap = sheet.cell(7,2)
        ws.write(row,0,name)
        ws.write(row,1,edges.value)
        ws.write(row,2,nodes.value)
